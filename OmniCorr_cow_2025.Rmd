...
```{r}
newP <- read_tsv("proteomics_log_raw_annotation_taxonomy 2.tsv")

newP %>%
  mutate(sample = paste(animal, timepoint, sep ="")) %>% 
  #filter(str_detect(sample, "T6")) %>% 
  select(protein, sample, intensity) %>% 
  ggplot(aes(x = log10(intensity+1))) +
  geom_histogram(bins = 100) +
  xlab("Overall proteomics")
...
annoD <- read_tsv("proteomics_log_raw_annotation_taxonomy 2.tsv") %>% 
  filter(str_detect(timepoint, "T6")) %>% 
  filter(str_detect(source, "D"))

# for liver and wall
annoL <- read_tsv("proteomics_log_raw_annotation_taxonomy 2.tsv") %>% 
  filter(str_detect(timepoint, "T6"))%>% 
  filter(str_detect(source, "L"))

annoW <- read_tsv("proteomics_log_raw_annotation_taxonomy 2.tsv") %>% 
  filter(str_detect(timepoint, "T6")) %>% 
  filter(str_detect(source, "W"))
...
for(i in seq_along(net_results)){
      ...
      group_annotated <- stack(net_results[[group_index_value]]$colors) %>% 
        ...
        left_join(annotations, by = "protein") %>%  # Ensure you join by the correct column
        group_by(module) %>%
        drop_na(KEGG_ko) %>%
...
for (i in 1:length(master_object_names)) {
  ...
  all_frames[[i]] <- current_frame
   ...
   # You can perform other operations here using `current_object`
}
...
```{r}
save(meta, newP, combined_MAG_genome, metadata, groups, annoD, annoL, annoW, kegg_data_raw, file = "Omnicorr_cow.RData")
```
