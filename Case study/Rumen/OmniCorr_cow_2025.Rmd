...
newP_slim <- readRDS("proteomics_slim.rds")
```
...
##### feel free to skip this chunk, for testing purpose only.
...
```{r pca}
meta <- meta %>%
rownames_to_column("sample") %>% left_join(data.frame(sample = colnames(ciliates)), by = "sample") %>% column_to_rownames("sample")
...
# in case original column still "chr", run below again.... 
meta_cln <- meta_cln %>%
  mutate(
    Breed = as.numeric(factor(Breed)),
    ch4_dmi_binary = as.numeric(factor(ch4_dmi_binary, levels = c("low", "high")))
  )
